...
/*        //上传文件分类
        $extra_params = $this->request->post('extra_params');

        if($extra_params){
            $fileSaveName    = (empty($app) ? '' : $app . '/') .$extra_params.'/' . $strDate . '/' . md5(uniqid()) . "." . $strFileExtension;
        }else{
...
//        }
      ...
      /*  return [
            ...
            "name"        => $arrInfo["filename"],
            'id'          => $strId,
            'preview_url' => cmf_get_root() .'/upload/' . $fileSaveName,
            'url'         => cmf_get_root() . $fileSaveName,
        ];*/
        return [
            'filepath'    => $arrInfo["file_path"],
            "name"        => $arrInfo["filename"],
