...
class DatabaseController extends AdminBaseController
{
    ...
    public function revert($time = 0, $part = null, $start = null) {
        if (is_numeric($time) && is_null($part) && is_null($start)) { //初始化
            //获取备份文件信息
            $name = date('YmdHis', $time) . '-*.sql*';
            cmf_log_record($name);  //添加日志 $id=对象ID
            $path = realpath(Config::get('data_backup_path')) . DIRECTORY_SEPARATOR . $name;
            $files = glob($path);
            $list = [];
