 ...
 */
class FloorInfoController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        if ($b_mark) {
            $where['b_mark'] = $b_mark;
            $place=array('mark'=>$b_mark);
        }else{
            $place=array('mark'=>"");
        }

        $place_data = Db::name('building_info')->where(["status"=>1])->select();
        $this->assign("place_data",$place_data);
        $this->assign("place",$place);
        $floor_info=Db::table("fn_floor_info")
            ->alias("b")
            ->where($where)
            ->field("b.*,p.name as building_name")
            ->join("fn_building_info p","b.b_mark = p.mark",'left')
            ->order("create_time DESC,b.list_order AES")
            ->paginate(10);
        // 获取分页显示
        $page = $floor_info->render();
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $result = $this->validate($this->request->param(), 'FloorInfo');
            if ($result !== true) {
                $this->error($result);
            } else {
                $_POST['create_time'] = date('Y-m-d H:i:s', time());
                $result = DB::name('floor_info')->insertGetId($_POST);
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("添加成功！", url("FloorInfo/index"));
                } else {
                    $this->error("添加失败！");
                }
            }
    ...
    public function edit()
    {
        ...
        $data = DB::name('floor_info')->where(["id" => $id])->find();
    ...
    public function editPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                $result = DB::name('floor_info')->update($_POST);
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("保存成功！", url("FloorInfo/index"));
                } else {
                    $this->error("保存失败！");
                }
            }
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            ...
            if (Db::name('floor_info')->delete($id) !== false) {
                cmf_log_record($id);  //添加日志 $id=对象ID
                $this->success("删除成功！");
        ...
        if (isset($param['ids'])) {
            ...
            $userInfo = Db::name('floor_info')->where(['id' => ['in', $ids]])->select();
            if (Db::name('floor_info')->where(['id' => ['in', $ids]])->delete() !== false) {
    ...
    {
        parent::listOrders(Db::name('floor_info'));
        $this->success("排序更新成功！", '');
    }
    /**
     * 启用/未启用
    ...
    public function toggle(){
        ...
        if(isset($param['ids'])  && isset($param['display'])){
            $ids = $this->request->param('ids/a');
            if (Db::name('floor_info')->where(['id' => ['in', $ids]])->update(['status' =>1])!==false) {
                $this->success("启用成功！");
            } else {
                $this->error("启用失败！");
            }
        ...
        if(isset($param['ids']) && isset($param['hide'])){
            $ids = $this->request->param('ids/a');
            if (Db::name('floor_info')->where(['id' => ['in', $ids]])->update(['status' =>0])!==false) {
                $this->success("禁用成功！");
            } else {
                $this->error("禁用失败！");
            }
    ...
    public function upload_field_excel_post(){
        ...
        if($Upload){
                ...
                if(file_exists($filePath))	 { //检查文件是否存在
                    ...
                    for($i=2;$i<=$highestRow;$i++){
                        ...
                        if($name){
                            ...
                            $response= DB::name('floor_info')->insertGetId($data);;
                            if($response!==false){
                                $a++;
                            }
                        }else{
            ...
            if(!empty($file_url)){
                ...
                //如果文件存在
                if(file_exists($filePath))	 { //检查文件是否存在
                    $sql = "TRUNCATE TABLE ". config("database.prefix")."floor_info";
