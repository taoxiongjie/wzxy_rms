...
class AssetTypeController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'AssetType');
            if ($result !== true) {
                $this->error($result);
            } else {
                ...
                if ($result !== false) {
                    $this->success("添加成功！", url("AssetType/index"));
                } else {
                    $this->error("添加失败！");
                }
            }
    ...
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'AssetType.edit');
            if ($result !== true) {
                // 验证失败 输出错误信息
                $this->error($result);
            } else {
                ...
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("保存成功！", url("AssetType/index"));
                } else {
                    $this->error("保存失败！");
                }
            }
