...
class RbacController extends AdminBaseController
{
     ...
     * )
     */
    public function portal_authorize()
    {
        $content = hook_one('admin_rbac_authorize_view');
        ...
        $result = $portalMenuModel->menuCache();

        $newMenus      = [];
        $privilegeData = $AuthAccess->where(["role_id" => $roleId ,"type"=>'portal_url'])->column("rule_name");//获取权限表数据
     ...
     * )
     */
    public function portal_authorizePost()
    {
        if ($this->request->isPost()) {
            $roleId = $this->request->param("roleId", 0, 'intval');
            if (!$roleId) {
            ...
            if (is_array($this->request->param('menuId/a')) && count($this->request->param('menuId/a')) > 0) {
                ...
                Db::name("authAccess")->where(["role_id" => $roleId, 'type' => 'portal_url'])->delete();
                foreach ($_POST['menuId'] as $menuId) {
                    $menu = Db::name("portalMenu")->where(["id" => $menuId])->field("app,controller,action")->find();
                    if ($menu) {
                        $app    = $menu['app'];
                        $model  = $menu['controller'];
                        $action = $menu['action'];
                        $name   = strtolower("$app/$model/$action");
                        Db::name("authAccess")->insert(["role_id" => $roleId, "rule_name" => $name, 'type' => 'portal_url']);
                    }
                }

                cache(null, 'portal_menus');// 删除后台菜单缓存
