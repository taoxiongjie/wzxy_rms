...
/**
 * Class RepairManageController
 * @package app\admin\controller
 */
class RepairManageController extends AdminBaseController
{
    ...
    public function index()
    {
       ...
       /* $request = input('request.');
        ...
        $repair_manage_info=Db::table("fn_repair_manage")
            ->alias("r")
        ...
        // 获取分页显示
        $page = $repair_manage_info->render();

        $this->assign("repair_manage_info",$repair_manage_info);
    ...
    public function add()
    {
        ...
        $repair_manage_info=Db::table("fn_asset_info")
            ->alias("a")
            ->whereOr($keywordComplex)
            ->where($where)
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'RepairManage.add');
            if ($result !== true) {
                $this->error($result);
            } else {
                $_POST['create_time'] = date('Y-m-d H:i:s', time());
                $_POST['user_id'] =cmf_get_current_admin_id();
                $result = DB::name('repair_manage')->insertGetId($_POST);
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("添加成功！", url("RepairManage/index"));
                } else {
                    $this->error("添加失败！");
                }
            }
    ...
    public function edit()
    {
        ...
        $data=Db::table("fn_repair_manage")
            ->alias("r")
            // ->whereOr($keywordComplex)
            ->where(["r.id" => $id])
    ...
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'RepairManage');
            if ($result !== true) {
                // 验证失败 输出错误信息
                $this->error($result);
            } else {
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            $id = $this->request->param('id', 0, 'intval');
            if (Db::name('repair_manage')->delete($id) !== false) {
                cmf_log_record($id);  //添加日志 $id=对象ID
        ...
        if (isset($param['ids'])) {
            ...
            if (Db::name('repair_manage')->where(['id' => ['in', $ids]])->delete() !== false) {
    ...
    {
        parent::listOrders(Db::name('repair_manage'));
        $this->success("排序更新成功！", '');
    }
    /**
     * 维修处理
    ...
    public function handle_post(){
        ...
        if (isset($param['ids'])) {
            if ($this->request->isPost()) {
                $resultva = $this->validate($this->request->param(), 'RepairManage');
                if ($resultva !== true) {
                    // 验证失败 输出错误信息
                    $this->error($resultva);
                } else {
                    ...
                    foreach ($idArr as $val) {
                        ...
                        $result                = Db::name('repair_manage')->insertGetId($data);
                        if ($result) {
                    ...
                    if ($a == count($idArr)) {
                        $this->success("操作成功！", url("RepairManage/add"));
                    } else {
                        $this->error("操作失败！");
                    }
                }
    ...
    public function toggle(){
        ...
        if(isset($param['ids'])  && isset($param['display'])){
            ...
            if (Db::name('repair_manage')->where(['id' => ['in', $ids]])->update(['status' =>1,'update_time'=>$update_time,'user_id'=>$user_id])!==false) {
                $this->success("已修复成功！");
            } else {
                $this->error("已修复失败！");
            }
        ...
        if(isset($param['ids']) && isset($param['hide'])){
            ...
            $update_time=date('Y-m-d H:i',time());
            $user_id=cmf_get_current_user_id();
            if (Db::name('repair_manage')->where(['id' => ['in', $ids]])->update(['status' =>0,'update_time'=>$update_time,'user_id'=>$user_id])!==false) {
                $this->success("报废成功！");
            } else {
                $this->error("报废失败！");
            }
        ...
        if(isset($param['ids']) && isset($param['repair'])){
            ...
            $update_time=date('Y-m-d H:i',time());
            $user_id=cmf_get_current_user_id();
            if (Db::name('repair_manage')->where(['id' => ['in', $ids]])->update(['status' =>2,'update_time'=>$update_time,'user_id'=>$user_id])!==false) {
                $this->success("维修成功！");
            } else {
                $this->error("维修失败！");
            }
            ...
            if(!empty($file_url)){
                ...
                if(file_exists($filePath))	 { //检查文件是否存在
                    ...
                    for($i=2;$i<=$highestRow;$i++){
                        ...
                        $data['status']=$status;
                        if(DB::name('repair_manage')->where(array('id'=>$id))->count()){
                            $response= DB::name('repair_manage')->where(array('id'=>$id))->update($data);;
                            if($response!==false){
                                $a++;
                            }else{
                                $b++;
                            ...
                            $response= DB::name('repair_manage')->add($data);
                            if($response!==false){
                                $a++;
                            }else{
                                $b++;
    ...
    public function download_RepairManage()
    {
        //先清空缓存  避免表字段信息有残留
        cmf_clear_cache();
        ...
        $repair_manage= DB::name('repair_manage')->field('id,name,asset_type,building_name,floor_name,room_name,manufact,asset_num,purchaser,purchase_amount,purchase_time,supplier,repair_time,power,weight,remark,list_order,status')->select();
        $repair_manage = $repair_manage->toArray();
        $filename='资产信息数据表文件';
        ...
        $column = 2;
        foreach($repair_manage as $key => $rows){ //行写入
            $span = ord("A");
