...
use app\admin\model\RepairInfoModel;
use think\Db;

/**
 * Class RepairInfoController
 * @package app\admin\controller
 */
class RepairInfoController extends AdminBaseController
{
    ...
    public function index()
    {
       ...
       /* $request = input('request.');
        ...
        $repair_info_info=Db::table("fn_repair_info")
            ->alias("r")
        ...
        // 获取分页显示
        $page = $repair_info_info->render();

        $this->assign("repair_info_info",$repair_info_info);
    ...
    public function add()
    {
        ...
        $repair_info_info=Db::table("fn_asset_info")
            ->alias("a")
            ->whereOr($keywordComplex)
            ->where($where)
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'RepairInfo.add');
            if ($result !== true) {
                $this->error($result);
            } else {
                $_POST['create_time'] = date('Y-m-d H:i:s', time());
                $_POST['user_id'] =cmf_get_current_admin_id();
                $result = DB::name('repair_info')->insertGetId($_POST);
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("添加成功！", url("RepairInfo/index"));
                } else {
                    $this->error("添加失败！");
                }
            }
    ...
    public function edit()
    {
        ...
        $data=Db::table("fn_repair_info")
            ...
            ->join("fn_user u","r.user_id = u.id",'left')
            ->find();
        $RepairInfoModel = new RepairInfoModel();
        $this->assign("arrStatus", $RepairInfoModel::$STATUS);
        $this->assign($data);
    ...
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'RepairInfo');
            if ($result !== true) {
                // 验证失败 输出错误信息
                $this->error($result);
            } else {
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            $id = $this->request->param('id', 0, 'intval');
            $repair_info=Db::name('repair_info')->where(['id' => $id])->find();
            if (Db::name('repair_info')->delete($id) !== false) {
                cmf_log_record($id);  //添加日志 $id=对象ID
        ...
        if (isset($param['ids'])) {
            ...
            $repair_info=Db::name('repair_info')->where(['id' => ['in', $ids]])->select();
            if (Db::name('repair_info')->where(['id' => ['in', $ids]])->delete() !== false) {
    ...
    {
        parent::listOrders(Db::name('repair_info'));
        $this->success("排序更新成功！", '');
    }
    /**
     * 维修处理
    ...
    public function handle_post(){
        ...
        if (isset($param['ids'])) {
            if ($this->request->isPost()) {
                $resultva = $this->validate($this->request->param(), 'RepairInfo');
                if ($resultva !== true) {
                    // 验证失败 输出错误信息
                    $this->error($resultva);
                } else {
                    ...
                    foreach ($idArr as $val) {
                        ...
                        $result                = Db::name('repair_info')->insertGetId($data);
                        if ($result) {
                            $result_asset_info = DB::name('asset_info')->where(['id' => $val])->update(['status' => 2]);
                    ...
                    if ($a == count($idArr)) {
                        $this->success("操作成功！", url("RepairInfo/add"));
                    } else {
                        $this->error("操作失败！");
                    }
                }
    ...
    public function toggle(){
        ...
        if(isset($param['ids'])  && isset($param['display'])){
            ...
            if (Db::name('repair_info')->where(['id' => ['in', $ids]])->update(['status' =>1,'update_time'=>$update_time,'user_id'=>$user_id])!==false) {
                $this->success("已修复成功！");
            } else {
                $this->error("已修复失败！");
            }
        ...
        if(isset($param['ids']) && isset($param['hide'])){
            ...
            $update_time=date('Y-m-d H:i',time());
            $user_id=cmf_get_current_user_id();
            if (Db::name('repair_info')->where(['id' => ['in', $ids]])->update(['status' =>0,'update_time'=>$update_time,'user_id'=>$user_id])!==false) {
                $this->success("报废成功！");
            } else {
                $this->error("报废失败！");
            }
        ...
        if(isset($param['ids']) && isset($param['repair'])){
            ...
            $update_time=date('Y-m-d H:i',time());
            $user_id=cmf_get_current_user_id();
            if (Db::name('repair_info')->where(['id' => ['in', $ids]])->update(['status' =>2,'update_time'=>$update_time,'user_id'=>$user_id])!==false) {
                $this->success("维修成功！");
            } else {
                $this->error("维修失败！");
            }
            ...
            if(!empty($file_url)){
                ...
                if(file_exists($filePath))	 { //检查文件是否存在
                    ...
                    for($i=2;$i<=$highestRow;$i++){
                        ...
                        $data['status']=$status;
                        if(DB::name('repair_info')->where(array('id'=>$id))->count()){
                            $response= DB::name('repair_info')->where(array('id'=>$id))->update($data);;
                            if($response!==false){
                                $a++;
                            }else{
                                $b++;
                            ...
                            $response= DB::name('repair_info')->add($data);
                            if($response!==false){
                                $a++;
                            }else{
                                $b++;
    ...
    public function download_RepairInfo()
    {
        //先清空缓存  避免表字段信息有残留
        cmf_clear_cache();
        ...
        $repair_info= DB::name('repair_info')->field('id,name,asset_type,building_name,floor_name,room_name,manufact,asset_num,purchaser,purchase_amount,purchase_time,supplier,repair_time,power,weight,remark,list_order,status')->select();
        $repair_info = $repair_info->toArray();
        $filename='资产信息数据表文件';
        ...
        $column = 2;
        foreach($repair_info as $key => $rows){ //行写入
            $span = ord("A");
