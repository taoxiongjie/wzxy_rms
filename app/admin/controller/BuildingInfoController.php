 ...
 */
class BuildingInfoController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'BuildingInfo');
            if ($result !== true) {
                $this->error($result);
            } else {
                $_POST['create_time'] = date('Y-m-d H:i:s', time());
                $result = DB::name('building_info')->insertGetId($_POST);
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("添加成功！", url("BuildingInfo/index"));
                } else {
                    $this->error("添加失败！");
                }
            }
    ...
    public function edit()
    {
        ...
        $data = DB::name('building_info')->where(["id" => $id])->find();
        $TagModel = new TagModel();
        $this->assign("arrStatus", $TagModel::$STATUS);
        $this->assign($data);
        $this->assign("data",$data);
    ...
    {
        if ($this->request->isPost()) {
            $result = $this->validate($this->request->param(), 'BuildingInfo.edit');
            if ($result !== true) {
                // 验证失败 输出错误信息
                $this->error($result);
            } else {
                $result = DB::name('building_info')->update($_POST);
                cmf_log_record($result);  //添加日志 $id=对象ID
                if ($result !== false) {
                    $this->success("保存成功！", url("BuildingInfo/index"));
                } else {
                    $this->error("保存失败！");
                }
            }
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            $id = $this->request->param('id', 0, 'intval');
            if (Db::name('building_info')->delete($id) !== false) {
                cmf_log_record($id);  //添加日志 $id=对象ID
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
        ...
        if (isset($param['ids'])) {
            ...
            if (Db::name('building_info')->where(['id' => ['in', $ids]])->delete() !== false) {
    ...
    {
        parent::listOrders(Db::name('building_info'));
        $this->success("排序更新成功！", '');
    }
    /**
     * 启用/未启用
    ...
    public function toggle(){
        ...
        if(isset($param['ids'])  && isset($param['display'])){
            $ids = $this->request->param('ids/a');
            if (Db::name('building_info')->where(['id' => ['in', $ids]])->update(['status' =>1])!==false) {
                $this->success("启用成功！");
            } else {
                $this->error("启用失败！");
            }
        ...
        if(isset($param['ids']) && isset($param['hide'])){
            $ids = $this->request->param('ids/a');
            if (Db::name('building_info')->where(['id' => ['in', $ids]])->update(['status' =>0])!==false) {
                $this->success("禁用成功！");
            } else {
                $this->error("禁用失败！");
            }
    ...
    public function upload_field_excel_post(){
        ...
        if($Upload){
                ...
                if(file_exists($filePath))	 { //检查文件是否存在
                    ...
                    for($i=2;$i<=$highestRow;$i++){
                        ...
                        if($name){
                            ...
                            $response= DB::name('building_info')->insertGetId($data);;
                            if($response!==false){
                                $a++;
                            }
                        }else{
            ...
            if(!empty($file_url)){
                ...
                //如果文件存在
                if(file_exists($filePath))	 { //检查文件是否存在
                    $sql = "TRUNCATE TABLE ". config("database.prefix")."building_info";
